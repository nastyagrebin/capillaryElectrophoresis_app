...
class AlignmentController:
    """
    Semi-manual anchor selection and alignment (Panel + Bokeh) with CSV export.
    Exposes:
      - input_by_sample: {name: DataFrame(time, intensity)}
      - pseudotimes_df: DataFrame of aligned pseudotimes after Align.
    """
    def __init__(self):
        # ---------- Data in/out ----------
        ...
        self._N: int = 0
        # anchors_per_sample[name] = [(x1,y1), (x2,y2), ...]
        ...
        # Targets (editable “anchor values”)
        ...
        # ---------- Layout ----------
        self._figure_row_placeholder = pn.Row(sizing_mode="stretch_width")  # replaced dynamically
        self._exports_row = pn.Row(
            pn.Column(self.csv_name, self.csv_download),
            sizing_mode="stretch_width",
        )
        ...
        self.section = pn.Column(
            ...
            self.status,
            sizing_mode="stretch_width",
            visible=True,   # shown via set_input()
        )
        
                # ---- notify app when alignment finishes ----
        self._aligned_listeners = []  # list[Callable[[Dict[str, pd.DataFrame]], None]]
        self.aligned_by_sample: Dict[str, pd.DataFrame] = {}


    # ---------- External API ----------
    def set_input(self, by_sample: Dict[str, pd.DataFrame]) -> None:
        """Called by app.py after normalization Apply/Skip."""
        ...
        # enable UI for fresh session
        self.start_btn.disabled = False
        self.use_sel_btn.disabled = True
        self.done_anchor_btn.disabled = True
        self.finalize_btn.disabled = False  # validate before running

        # per-sample UI options
    ...
    def _build_selection_figure(self):
        ...
        p = bokeh.plotting.figure(
            ...
            tools="pan,box_select,box_zoom,reset,save,hover",  # no wheel zoom by default
        ...
        for i, s in enumerate(self._samples):
            ...
            # points (transparent) used to capture selection (indices)
    ...
    def _on_start(self, _=None):
        ...
        # Prime targets with evenly spaced values across [0,1]
    ...
    def _on_finalize(self, _=None):
        ...
        # compute alignment (requires CEtools)
        try:
        ...
        aligned = bokeh.plotting.figure(
            ...
            x_range=(0.0, 1.0),
            tools="pan,box_zoom,reset,save,hover",  # no wheel zoom by default
        )
        aligned.legend.location = "right"
        aligned.legend.click_policy = "hide"
        aligned.hover.tooltips = [("pt","$x{0.000}"), ("y","$y{0.000}")]
        pt_df = pd.DataFrame(index=self.input_by_sample[self._samples[0]].index)
        ...
        self.pseudotimes_df = pt_df
        self.aligned_fig = aligned
        self.aligned_pane.object = aligned
        
        # ---- Build aligned dataset on a common pseudotime grid and notify listeners ----
        # Common grid length = original time length
        N = len(t)  # original number of points
        grid = np.linspace(0.0, 1.0, N)

        aligned_by_sample = {}
        for s in self._samples:
            pseudo = self.pseudotimes_df[s].to_numpy()    # length N
            y      = self.input_by_sample[s]["intensity"].to_numpy()

            # Ensure monotonic pseudotime for interpolation; if not, stable sort by pseudo
            order = np.argsort(pseudo)
            pseudo_sorted = pseudo[order]
            y_sorted      = y[order]

            # Remove duplicates in pseudotime (np.interp requires ascending unique x)
        ...
        # Notify any listeners (e.g., the NMF tab in app.py)
    ...
    def add_aligned_listener(self, fn):
    ...
    def _csv_bytes(self):
        ...
        for s in samples:
            ...
            out[s]         = self.input_by_sample[s]["intensity"].to_numpy()
