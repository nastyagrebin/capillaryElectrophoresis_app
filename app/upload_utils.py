...
# ---- CSV Loader (supports narrow and wide) ----
def load_csv_bytes_to_dfs(file_name: str, data: bytes) -> Dict[str, pd.DataFrame]:
    import io
    df = pd.read_csv(io.BytesIO(data))
    if "time" not in df.columns:
        # Try case-insensitive search for time
        time_cols = [c for c in df.columns if c.lower() == "time"]
        if time_cols:
            df = df.rename(columns={time_cols[0]: "time"})
        else:
            raise ValueError(f"File {file_name} is missing a 'time' column.")

    # format 1: "narrow" (time, intensity)
    if "intensity" in df.columns:
        sample_name = sanitize_name(file_name)
        return {sample_name: df[["time", "intensity"]]}
    
    # format 2: "wide" (time, sample1, sample2, ...)
    if len(df.columns) > 1:
        out = {}
        for col in df.columns:
            if col == "time":
                continue
            sample_df = pd.DataFrame({"time": df["time"], "intensity": df[col]})
            out[sanitize_name(col)] = sample_df
        return out
    
    raise ValueError(f"File {file_name} does not have an 'intensity' column or multiple sample columns.")
