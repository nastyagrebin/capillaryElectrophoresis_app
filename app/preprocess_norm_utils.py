...
class NormalizationController:
    ...
    preproc_offset: pn.widgets.FloatSlider = field(default_factory=lambda: pn.widgets.FloatSlider(name="Vertical offset (preview)", start=0.0, end=10.0, step=0.5, value=0.0, sizing_mode="stretch_width"))
    asinh_toggle: pn.widgets.Checkbox = field(default_factory=lambda: pn.widgets.Checkbox(name="Use asinh transform (preview)", value=True))

    post_offset: pn.widgets.FloatSlider = field(default_factory=lambda: pn.widgets.FloatSlider(name="Vertical offset (normalized)", start=0.0, end=1.0, step=0.05, value=0.0, sizing_mode="stretch_width"))
    post_asinh: pn.widgets.Checkbox = field(default_factory=lambda: pn.widgets.Checkbox(name="Use asinh transform (normalized)", value=True))
    ...
    def __post_init__(self):
        ...
        self.asinh_toggle.param.watch(self._on_params_changed, "value")
        self.post_offset.param.watch(lambda _: self._update_after_plot(), "value")
        self.post_asinh.param.watch(lambda _: self._update_after_plot(), "value")
    ...
    def _update_after_plot(self):
        ...
        t = "Normalized curves" if self.apply_btn.disabled else "Normalized curves preview" # heuristic
        try:
    ...
    def _render_before_fig(self):
        ...
        for i, (name, df) in enumerate(self.current_by_sample.items()):
            ...
            # Apply transforms for DISPLAY Source
            y_disp = y.copy()
            if ash:
                y_disp = np.arcsinh(y_disp)
            y_disp = y_disp - i * off

            src = ColumnDataSource(dict(x=x, y=y_disp))
            fig.line("x","y", source=src, color=colors[i % len(colors)], legend_label=str(name)[:12], line_width=2)
            ...
            cb = CustomJS(args=dict(
                ...
                const points_idx = [];
                for (let i=0;i<x.length;i++) if (x[i]>=xmin && x[i]<=xmax) points_idx.push(i);
                if (points_idx.length === 0) { auc_src.change.emit(); return; }

                // Apex
                let best = -1, besty = -Infinity;
                for (const i of points_idx) { if (y[i] > besty) { besty = y[i]; best = i; } }
                ...
                // Build polygon to baseline (which is now - (sample_idx * offset_val))
                const px = [], py = [];
                const base_y = - (sample_idx * offset_val);
                for (let i=L;i<=R;i++) { px.push(x[i]); py.push(y[i]); }
                for (let i=R;i>=L;i--) { px.push(x[i]); py.push(base_y); }
