...
# Local utils
from upload_utils import sanitize_name, convert_cdf_bytes_to_df, try_merge_same_time
...
from nmf_utils import build_nmf_current_section, build_nmf_preload_section
...
preview_group = pn.Column(pn.pane.Markdown("### Chromatograph(s) preview"), plot_pane, offset_slider, visible=False, sizing_mode="stretch_width")
...
def _render_plot(*_):
    if not state.converted_by_sample:
        ...
        state.last_fig = None
        return
    fig = make_preview_plot(state.converted_by_sample, minutes=prefer_minutes.value, offset=offset_slider.value, title="Chromatograph(s) preview")
...
def _on_convert_click(event):
    ...
    try:
        _maybe_snapshot("(after convert)")
        TABS.active = 1  # Preprocess
    except Exception:
        pass
...
upload_tab = pn.Column(pn.Row(left_col, pn.layout.HSpacer(width=16), right_col, sizing_mode="stretch_width"), sizing_mode="stretch_width")
...
# ===================== NMF tabs =====================
nmf_current_section, nmf_current_ctrl = build_nmf_current_section()    # Alignment-driven
nmf_preload_section, nmf_preload_ctrl = build_nmf_preload_section()    # CSV-driven
nmf_current_section.visible = True
nmf_preload_section.visible = True

def _nmf_preload_aligned_imported(P: pd.DataFrame, Y: pd.DataFrame, rows_are_traces: bool):
    state.aligned_pseudotimes_df = P.copy()
    state.aligned_norm_df = Y.copy()
    state.rows_are_traces_aligned = bool(rows_are_traces)
nmf_preload_ctrl.on_aligned_imported = _nmf_preload_aligned_imported
...
def _unlock_viz_diversity_after_nmf(H_df: pd.DataFrame):
    """
    Called after either NMF tab completes. Makes Viz ready and ensures Diversity keeps its aligned inputs.
    """
    ...
    try:
        ...
        pass
    # Diversity keeps aligned inputs (so users can compute metrics), no set_H call here
    try:
...
def _alignment_done_callback(pseudotimes_df: pd.DataFrame, norm_df: pd.DataFrame, *, rows_are_traces: bool):
    ...
    try:
        nmf_current_ctrl.set_alignment_input(pseudotimes_df, norm_df, rows_are_traces=rows_are_traces)
        TABS.active = 3  # NMF (current)
        bridge_status.object = ok("Alignment complete → NMF primed (current).")
    except Exception as e:
...
# ---------- When either NMF finishes, unlock viz/diversity ----------
...
try:
    nmf_current_ctrl.on_done  = _nmf_done_callback
    nmf_preload_ctrl.on_done  = _nmf_done_callback
except Exception:
    pass
...
TABS = pn.Tabs(
    ...
    ("NMF", nmf_current_section),        # current session only
    ("NMF (preload)", nmf_preload_section),  # CSV-only
