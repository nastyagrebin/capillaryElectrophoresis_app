...
class SmoothController:
    def __init__(self):
        ...
        self.window = pn.widgets.IntSlider(name="Savgol window (odd)", start=5, end=101, step=2, value=21)
    ...
    def _on_preview_btn_click(self, _):
        self._calculate_smoothing()
        self._update_plots()
    ...
    def _update_plots(self):
        ...
        b = plot_multi(self.input_by_sample, "Before smoothing (input)", offset=off, asinh=ash)
        
        current_out = self.smoothed_by_sample if self.smoothed_by_sample else self.input_by_sample
        a = plot_multi(current_out, "After smoothing", offset=off, asinh=ash)
        
        a.x_range = b.x_range; a.y_range = b.y_range
        self.before_pane.object = b; self.after_pane.object = a
        
        if self.smoothed_by_sample:
            self.apply_btn.disabled = False
            self.export_btn.disabled = False
            self.export_status.object = ok("Smoothed data ready to export.")
            self.status.object = ok("Smoothing preview updated.")
